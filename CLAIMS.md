...
The forward-looking experimental program (EXP-002 … EXP-019) — each experiment
...
| B3 | hashrope branch/snapshot beats a faithful PagedAttention block-table COW reimplementation (Kwon et al., SOSP 2023; block-table/ref-count structure **§4.2**, copy-on-write **§4.4**) on ToT branching beyond a **branch-creation** crossover N* — O(log w) branch-creation and O(B·log N) branching memory vs O(N/block) block-table copy — with byte-identical branched contexts | SUPPORTED (Milestone A + B) | **EXP-019 Milestone A** (confirmatory, 2026-06-13, commit 5848110, 3 seeds × 3 invocations = 9 runs, real gpt2-tokenized corpus, hashrope 0.2.2). Baseline = faithful PagedAttention block-table COW reimplementation (`src/paged_attention_cow.py`, §4.2/§4.4), 55 unit tests. Gated metric = **branch-creation** (fork + first divergent step), locked pre-data in LOGBOOK Addendum A — bare fork has no crossover (hashrope's O(1) root-share wins at every N because divergence is deferred), so gating it would be reviewer-vulnerable. **All four clauses met:** (i) HARD cross-arm byte-identity hashrope==PagedAttention==oracle, **0 mismatches**; (ii) deterministic guards (hashrope per-branch new nodes ≤ ⌈log₂ leaves⌉+3; paged fork entries == ⌈N/B⌉); (iii) branch-creation crossover **N*=16k tokens** at reference block 16, monotone below; (iv) at N=1M, every block size {8,16,32,64}: 9/9 paired sign wins (p=0.004), branch-creation speedup **169×/97×/43×/20×**, branching-memory compression at 5 branches **613×/306×/153×/68×**. hashrope branch-creation is flat ~26→37 µs across N (O(log w)); PagedAttention scales linearly 14 µs→3.6 ms. **Honest boundaries reported in full:** per-token append — PagedAttention wins ~18× (O(1) amortized vs O(log w)); bare fork — hashrope O(1) ~39 ns flat vs PagedAttention up to 6.1 ms (linear). Results `experiments/exp_019_branch/results/exp019_milestoneA_latest.json`; figures `figures/exp019_{branch_create_latency,branch_memory_compression,boundary}.{png,pdf}`. **Caveat (carried to paper):** latencies are pure-Python interpreter-amplified; transferable claims = the deterministic structural guards + O(log w) vs O(⌈N/B⌉) scaling. **Milestone B (real gpt-oss-120b Game-of-24 ToT traces, recorded + replayed) ECOLOGICALLY CORROBORATES B3** (2026-06-16, n=9 = 3 seeds x 3 invocations, ranks 901-925, 7,868 branch-creation expansions, replayed deterministically through the same harness): **0 mismatches** (byte-identical, both arms, every P and block); branch-creation **9/9 paired wins at EVERY block**, growing to **74.6x/38.9x/19.1x/9.6x** (b8/16/32/64) at P=256k; marginal branching-memory compression **17x-67x** across the deployment-standard block regime {8,16,32}; reference-block crossover confirmed at **N*=16k** with block-dependent per-block crossovers characterized as a result. Criterion (iv)'s >=10x memory bar is met at b8/b16/b32 (67x/34x/17x); at the largest, most paged-favorable block b64 the marginal compression is **9.6x -- a disclosed near-miss, reported in full, not retrofitted** (LOGBOOK 2026-06-16 clarifies the under-specified single-block crossover clause (iii); no threshold lowered). Results `experiments/exp_019_branch/results/exp019_milestoneB_latest.json`. Promoted on the pre-registered LOGBOOK EXP-019 criterion (Addendum A). |
