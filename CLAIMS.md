...
| ID | Claim | Status | Evidence / Notes |
|----|-------|--------|------------------|
| S1 | Feeding disjoint leaves to parallel tokenizers gives 4.12× GPU ingestion speedup | RETRACTED | The two pipelines produce **different model inputs**: independent 4KB-chunk tokenization diverges at 197/244 boundaries (+0.15% tokens, GPT-2), and the chunked output is a padded 2D batch never unpadded/concatenated or timed as one sequence. **Repair #2 (next):** boundary-snapped chunker + token-identity proof obligation, then honest re-measurement (expected ~3×). |
...
| C3 | Indices are handled safely for multibyte (UTF-8) content | PLANNED | Current HybridContext conflates byte and character indices; a split can bisect a UTF-8 sequence. Define and test the byte-index contract (or add char-aware handling). |
