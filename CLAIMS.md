...
| B3 | hashrope branch/snapshot beats a faithful PagedAttention block-table COW reimplementation (Kwon et al., SOSP 2023; block-table/ref-count structure **§4.2**, copy-on-write **§4.4**) on ToT branching beyond a fork-latency crossover N* — O(log N) fork & O(log N) branching memory vs O(N/block) block-table copy — with byte-identical branched contexts | IN-PROGRESS | **EXP-019** (kickoff 2026-06-13, LOGBOOK plan committed). Leg 2 competitive baseline; faithful cited reimplementation (`src/paged_attention_cow.py`), code public for review. Builds on M1/EXP-004 machinery; token granularity (4-byte-LE per EXP-017). **Milestone A** (controlled context-size sweep 1k–1M tok, block sizes {8,16,32,64}) supplies the headline — fork crossover + large-context fork-latency & branching-memory wins. **Milestone B** = real gpt-oss-120b Game-of-24 ToT traces (ecological validity, recorded + replayed). Honest boundaries reported in full (not the thesis): PagedAttention append is O(1) amortized; short-context fork favors PagedAttention. Verbatim promotion criterion in LOGBOOK EXP-019. |
