...
| 019 | B3 | **COMPETITIVE Leg 2:** branch/snapshot — hashrope vs faithful PagedAttention block-table COW (Kwon et al. SOSP'23; structure §4.2, COW §4.4) on ToT branching: controlled context-size sweep (1k–1M tokens, **PRIMARY/headline**) + real gpt-oss-120b Game-of-24 traces (ecological) | Add | local-CPU | **in-progress** | Leg 2 competitive baseline; faithful cited reimplementation (`src/paged_attention_cow.py`), code public for review. Builds on M1/EXP-004 machinery. Headline = fork crossover + large-context fork-latency & branching-memory wins across block sizes {8,16,32,64}. Honest boundaries (append O(1) amortized; short-context fork) reported in full, not the thesis. LOGBOOK plan committed 2026-06-13. |
