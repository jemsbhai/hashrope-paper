...
| 002 | S4 | Flatten fix (in-order leaf walk + `b"".join`) vs broken O(N log N) re-split | Sol+Exp | local-CPU + Rust | planned | At kickoff verify the library exposes in-order leaf/child traversal; if not, a minimal read-only helper is needed (decide lib-bump vs experiment-local). ~500× expected. |
...
| 005 | T3 | Prefix-reuse identification: LCP O(log² N) multi-seed timing table (CIs) + prefix-dedup hit-rate | Add | local-CPU | planned | The conceptual heart of the unification thesis; **feeds the EXP-015 isolation design.** |
