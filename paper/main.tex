...
\midrule
Ingestion & exact tokens & 0 boundary diverg. & 7.6x at 16-way \\
Flatten & $O(N)$ & 0 re-hash / re-alloc & 800x vs re-split \\
Branching & $O(B\log N)$ mem & $O(\log w)$ nodes/fork & 166x at 8\,MB \\
...
1.041 (naive). Repeated system prompts, few-shot exemplars, and
template headers are encoded once.
...
edits). One more honest cost: persistence retains dead path nodes under
in-place-style churn, about 2.7 additional retained nodes per doubling
...
shared logarithmic spine (Fig.~\ref{fig:branchmem}). An ecological
replay of Game-of-24 tree-of-thought traces~\cite{yao2023tot} from a
120B open-weight model corroborates the controlled sweep: 17x to 67x
memory across deployed block sizes, byte-perfect reconstruction, and
the same 16\,K crossover. Below the crossover the paged table is
faster, as expected for short contexts.
...
exact position, 105/105. Verified reuse here means verified.
