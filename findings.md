...
## EXP-019: Competitive branch/snapshot -- Milestone B (real gpt-oss-120b Game-of-24 ToT replay, ecological) -- B3 corroborated

**Date:** 2026-06-16 (confirmatory, Milestone B; supplementary ecological validity)
**Type:** competitive baseline (Leg 2 of unification thesis), ecological replay
**Status:** B3 SUPPORTED (Milestone A) + ECOLOGICALLY CORROBORATED (Milestone B)

**Setup:** real Tree-of-Thought search traces generated by gpt-oss-120b on Game-of-24
(ranks 901-925, beam b=5, depth 3), recorded once to JSONL
(`data/canonical/tot_traces_gptoss120b_s{42,43,44}.jsonl`; 75 trees, 7,943 nodes,
7,868 branch-creation expansions; depth histogram {1:1979, 2:4676, 3:1213}; token-seal
re-validated against run-env gpt2 at replay), then **replayed deterministically** through
the SAME branch/snapshot harness as Milestone A (`src/branch_bench.py`, `src/memory.py`
unchanged). Each non-root node = one branch-creation off its parent's reconstructed
context (prefix + puzzle + path-thoughts); prefix = real gpt2-tokenized corpus prefix,
P in {0, 4k, 16k, 64k, 256k}; blocks {8,16,32,64}, reference 16. Both arms token space,
4-byte LE, so token-concat == byte-concat exactly. 3 seeds x 3 invocations = 9 runs;
correctness/guards verified once per seed (deterministic replay; >=3 distinct trees per
rank, so n=9 is real). hashrope 0.2.2, i9-14900HX. Results
`experiments/exp_019_branch/results/exp019_milestoneB_latest.json`.

**Key result (win-first):** Replaying real LLM-generated ToT search ecologically
corroborates B3. hashrope reconstructs every node context **byte-identically** (0
mismatches, both arms, every P and block) and beats the faithful PagedAttention COW
baseline on branch-creation latency at **every** block size, the advantage growing
monotonically with context to **74.6x / 38.9x / 19.1x / 9.6x** (b8/b16/b32/b64) at
P=256k -- all **9/9 paired sign wins**. Marginal branching-memory compression on the
real <=5-survivor beam is **17x-67x** across the deployment-standard block regime
{8,16,32}. hashrope branch-creation is flat (~16-24 us, O(log w)); PagedAttention
scales linearly (O(ceil(N/B)), to 1.82 ms at P=256k b8).

**Branch-creation latency speedup (PagedAttention/hashrope), ref block 16, mean n=9:**

| P (tokens) | hashrope | PagedAttention | speedup (b16) | speedup (b8) |
|---|---|---|---|---|
| 0 | 16.0 us | 5.6 us | 0.35x | 0.40x |
| 4k | 17.5 us | 20.0 us | 1.14x | 2.14x |
| **16k** | 21.5 us | 67.1 us | **3.13x** | 7.81x |
| 64k | 23.3 us | 239 us | 10.3x | 20.9x |
| **256k** | **24.4 us** | **948 us** | **38.9x** | **74.6x** |

At P=256k, all four blocks 9/9 paired: 74.6x/38.9x/19.1x/9.6x (b8/b16/b32/b64).

**Marginal branching-memory compression (PagedAttention/hashrope, real beam <=5), n=9:**
at P=256k -- b8 67.2+/-0.6, b16 34.3+/-0.3, b32 17.1+/-0.2, b64 9.6+/-0.1
(seed-stable, per-seed within ~2%, invocation std ~0). hashrope holds the real beam in
~480 KB marginal vs PagedAttention 4.6-32 MB (b64..b8).

**Crossover (block-dependent -- a characterized result):** reference-block-16
band-crossover at **N*=16k** exactly (within one grid step of Milestone A's N*=16k).
Per-block band-crossovers: b8 at 4k, b16/b32 at 16k, b64 at 64k. hashrope's structural
sharing is block-size-agnostic; PagedAttention's per-fork cost is Theta(ceil(N/B)), so
smaller blocks cross earlier and compress more. This tells an operator exactly where the
tradeoff sits for their block config -- more informative than a flat universal.

**Structural guards (deterministic):** hashrope per-expansion new nodes <= ceil(log2 w)+3;
PagedAttention fork == ceil(parent_len/B), every expansion, all cells.

**Honest boundaries (reported in full, not the thesis):**
- *Small contexts* -- at P=0 (bare Game-of-24, ~5-token base) PagedAttention wins every
  block (hashrope's rope/hash constant does not amortize; speedup 0.30x-0.40x). It also
  wins at P=4k for b32/b64 and P=16k for b64. Full per-cell grid (every paged win) in
  `sub_crossover_paged_wins`. This is the expected sub-crossover regime; nobody reaches
  for a persistent content-indexed structure to branch a 500-token prompt.
- *Largest block (b64) memory* -- marginal compression is **9.6x**, just under the
  pre-registered >=10x bar (met with large margin at b8/b16/b32: 67x/34x/17x). Disclosed
  in full as the sensitivity tail of the explained block-size trend; not retrofitted. The
  LOGBOOK 2026-06-16 entry clarifies that criterion (iii) (single-block crossover) was
  under-specified -- corrected to per-block, no threshold lowered.

**Honesty note:** absolute us/ms latencies are pure-Python interpreter-amplified; the
transferable claims are the deterministic structural guards (content-independent) and
O(log w) vs O(ceil(N/B)) scaling, with smaller constants expected in Rust.
...
## EXP-015: GPU-side validation + energy attribution on a real 7B serving stack (S3) -- SUPPORTED

**Setup.** Two decoupled tracks on vLLM 0.8.5.post1 + Qwen/Qwen2.5-7B-Instruct-1M (4x A100-SXM4-40GB, tp=4, ai-panther/SLURM). Track A: engine prefix-cache {OFF, ON} as the only varied axis; GPU Joules (NVML energy counters), TTFT, throughput. Track B: content-identification arms (hashrope LCP, vendored SGLang RadixCache v0.1.17, numpy flat scan) run on CPU strictly outside the GPU energy window, over the same token streams. Regimes: R1 = realistic ShareGPT/LMSYS streams (~642-token median prefixes); R2 = controlled shared-prefix sweep L in {131072, 262144, 393216, 524288, 1000000}. n=9 everywhere (3 seeds x 3 process invocations), mean +/- sample std, exact paired sign tests. All numbers regenerate from exp015_analysis_summary_latest.json via committed scripts (analysis selftest 23/23, figures selftest 4/4).

**Correctness (the exactness win).** Reuse prefixes selected by hashrope, radix, and flat are token-identical in every cell: 0/1800 gate mismatches (R1 0/1440, R2 0/360). The gate is proven non-vacuous by 105/105 corrupted-cached-entry negative controls (flip one token at first/mid/last positions; every arm reports the exact corrupt position, e.g. position 999,999 of a 1,000,000-token prefix).

**Identification at the length the engine cannot serve (the R2 headline).** hashrope identification latency is flat across the entire sweep (44.4-46.3 ms: the O(log) signature), while the production radix matcher grows 10.68 -> 53.19 ms. At L=1,000,000 hashrope beats radix: 44.42 +/- 0.63 ms vs 53.19 +/- 3.78 ms, 1.20x faster, paired sign 9/9, one-sided p=0.002 -- and vanilla dense-attention vLLM cannot prefill that length at all (Track A gracefully skipped at 1M; the serving wall is the engine's, not hashrope's). The live-stack crossover L*' is bracketed in (524,288, 1,000,000], and EXP-017's independently pre-registered CPU-model prediction (~571k) lands inside the bracket: a quantitative prediction confirmed on a real serving stack.

**Engine caching energy, honestly attributed.** With caching ON the engine's own prefix cache reduces GPU J/token by 22.65x +/- 1.84 (R1) and 69.6x -> 277.6x monotonically across R2 L (TTFT 92.5 s -> 0.87 s at L=524,288); 9/9 paired, p=0.002 at every point; 0 cache-ON regressions. These reductions are the ENGINE's effect, explicitly not hashrope's -- this measurement replaces, with correct attribution, the prior draft's faked 62.7% figure (which never invoked hashrope; see CLAIMS.md S3). Identity-layer GPU neutrality holds by construction: one engine run per cell, the identifier never feeds the engine, so the identity layer adds exactly zero GPU work.

**Honest negatives (in full, not the lede).** (a) At realistic ~642-token prefixes radix is far faster (0.037 ms vs hashrope 17.46 ms); the win regime is long shared prefixes beyond L*'. (b) The numpy flat O(L) scan is the fastest single-query LCP at every L (0.677 ms at 1M) -- it maintains nothing; the functional comparison is vs radix, the matcher a real engine actually uses, and hashrope's value is the unification (the same persistent structure serving O(log) edits, branching, and verified content identity). (c) The identity layer is conditional-on-use: redundant inside a single engine that already radix-matches, valuable for multi-instance routing, cross-request dedup, and opaque-blob contexts. (d) A corpus provenance gap for seeds 43/44 (node-side divergence, grid-recorded hashes authoritative, seed-42 chain byte-verified) is documented in LOGBOOK 2026-07-18; not result-invalidating.

**Net.** S3 SUPPORTED (real-stack validation + energy-transparency + caching attribution); contingent F6 not triggered (structural). Criterion (i)-(vi) met verbatim. EXP-015 closed; with B1/B2/B3 already SUPPORTED, all four legs of the unification thesis now have real-system evidence, and the prefix-reuse leg has it on a live 7B serving stack at a length dense attention cannot reach.
